 ...
 *
 * Modified for ST7801 DSI Screen (410x502) and Hynitron CST820 Touch (/dev/input/event5)
 *
 ******************************************************************/
#include <unistd.h>
#include <pthread.h>
...
static void configure_simulator(int argc, char ** argv)
{
    ...
    /* 针对 410x502 ST7801 面板的默认分辨率配置 */
    settings.window_width = atoi(env_w ? env_w : "410");
    settings.window_height = atoi(env_h ? env_h : "502");
